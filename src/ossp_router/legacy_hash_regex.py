...
def _fnv_update(state: int, data: bytes) -> int:
    for byte in data:
        state ^= byte
        state = (state * _FNV_PRIME) & _UINT64_MASK
    return state


def _stable_hash(value: str) -> int:
    return _fnv_update(_FNV_OFFSET, value.encode("utf-8"))


# Tokens and n-grams repeat heavily across a batch, so memoizing their FNV
# digests and normalized forms removes most byte-level hashing work.  The
# caps bound memory on adversarially diverse inputs; overflow falls back to
# direct computation with identical results.
_DIGEST_CACHE_LIMIT = 1_000_000
_UNIGRAM_DIGESTS: dict = {}
_BIGRAM_DIGESTS: dict = {}
_BIGRAM_PREFIX_STATES: dict = {}
_TOKEN_NORMALIZATION: dict = {}
...
def signed_hash_bins(tokens: Sequence[str], hash_bins: int) -> list:
    """Accumulate signed word unigram/bigram counts into ``hash_bins`` slots."""

    bins = [0.0] * hash_bins
    mask = hash_bins - 1
    sign_bit = 1 << 63
    # Bin totals are signed occurrence counts, so aggregating repeated tokens
    # first and adding ±count once yields exactly the same integer-valued
    # floats as adding ±1 per occurrence.
    unigram_cache = _UNIGRAM_DIGESTS
    for token, count in Counter(tokens).items():
        digest = unigram_cache.get(token)
        if digest is None:
            digest = _stable_hash(f"w1:{token}")
            if len(unigram_cache) < _DIGEST_CACHE_LIMIT:
                unigram_cache[token] = digest
        bins[digest & mask] += -float(count) if digest & sign_bit else float(count)
    bigram_cache = _BIGRAM_DIGESTS
    prefix_cache = _BIGRAM_PREFIX_STATES
    for pair, count in Counter(zip(tokens, tokens[1:])).items():
        digest = bigram_cache.get(pair)
        if digest is None:
            # FNV-1a is sequential, so resuming from the cached state after
            # "w2:<left>\x1f" hashes only the right token's bytes while
            # producing the exact digest of the full joined string.
            left, right = pair
            state = prefix_cache.get(left)
            if state is None:
                state = _fnv_update(_FNV_OFFSET, f"w2:{left}\x1f".encode("utf-8"))
                if len(prefix_cache) < _DIGEST_CACHE_LIMIT:
                    prefix_cache[left] = state
            digest = _fnv_update(state, right.encode("utf-8"))
            if len(bigram_cache) < _DIGEST_CACHE_LIMIT:
                bigram_cache[pair] = digest
        bins[digest & mask] += -float(count) if digest & sign_bit else float(count)
    return bins
...
def raw_feature_vector(episode: Episode, hash_bins: int) -> Tuple[float, ...]:
    ...
    bins = signed_hash_bins(tokens, hash_bins)
...
def _linear(head: LinearHead, values: Sequence[float]) -> float:
    # map(operator.mul, ...) yields the same products in the same order as the
    # previous generator expression, so fsum returns a bit-identical result.
    return head.intercept + math.fsum(
        map(operator.mul, head.coefficients, values)
    )
